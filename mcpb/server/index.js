...
server.registerTool(
  "connaissance_documents_exclude",
  {
    description: "Manage the user EXCLUSION list for PAID processing (Mistral OCR + LLM summary). Paths (relative to ~/Documents/) on this list are skipped by transcribe-plan AND summarize. For big born-digital docs with no structural value, or sensitive files you don't want sent to an external service. Add/remove paths or add_from_file (one rel path per line, for a batch). list=true shows the current list without changes.",
    inputSchema: {
      add: z.array(z.string()).optional().describe("Paths (rel ~/Documents) to exclude."),
      remove: z.array(z.string()).optional().describe("Paths to remove from the list."),
      add_from_file: z.string().optional().describe("Add all paths from this file (one per line)."),
      list: z.boolean().default(false).describe("Show the current list without modifying."),
    },
  },
  async (args) => {
    const a = [];
    for (const r of (args.add || [])) a.push("--add", r);
    for (const r of (args.remove || [])) a.push("--remove", r);
    if (args.add_from_file) a.push("--add-from-file", args.add_from_file);
    if (args.list) a.push("--list");
    return runAndFormat("documents", "exclude", a);
  }
);
...
server.registerTool(
  ...
  {
    description: "Mistral re-pass WORKLIST, page-bounded for cost. DB-driven (doc_signals). Lists documents that need OCR (scanned PDFs, image-documents) and do NOT yet have a Mistral transcription — either a vision-local transcription to UPGRADE to Mistral's structured markdown, or (by default) a scanned doc with no transcription at all. VISION→MISTRAL CASCADE: a vision-local transcription only enters the worklist if its ocr_confidence is <= max_confidence (default 0.55 — Mistral reserved for where the VLM truly helps: dense forms, old scans); above → counted vision_ok_skip. Born-digital fusion transcriptions (ocr_kind born-digital, characters from the text layer) → vision_fusion_skip; pdf-text fallbacks → pdf_text_skip — both re-includable via include_born_digital. Born-digital PDFs WITHOUT a transcription stay excluded (born_digital_skip — process them for free with the CLI `documents ocr-local --born-digital`) unless include_born_digital=true (counted born_digital_included). Phantom duplicate rows (NFC/NFD/case variants) are deduped (counts.phantom_dupes). Documents over max_pages go to 'deferred'. Returns counts, estimated_pages and estimated_cost_usd (Mistral OCR 4 batch, $2/1000 pages), plus to_transcribe[] (each item carries source canonical + read_source SSD mirror + transcription target). Read-only except --output_file. INTENDED FLOW: call with output_file to write a manifest, then mistral-ocr ocr_batch_submit(files_from_json=manifest, preserve_paths=~/Documents) → ocr_batch_results(output=Transcriptions/Documents) → documents_register_batch(from_scan=manifest, ocr_engine='mistral'). The manifest's read_source makes mistral-ocr read from the SSD mirror (no iCloud download). Requires up-to-date signals (documents signals, schema v>=6) for pages + ocr_cache.",
    inputSchema: {
      ...
      no_dedup_content: z.boolean().optional().describe("Disable content (hash) dedup. Default: OCR each identical content once; exact duplicates get a copy of the representative's transcription (no re-OCR, no cost, file kept in place)."),
      include_born_digital: z.boolean().optional().describe("Also include born-digital PDFs and Vision fusion/pdf-text transcriptions (default false). Use to unify the whole base on Mistral markdown."),
      max_confidence: z.number().optional().describe("Vision→Mistral cascade threshold: only upgrade a vision-local transcription whose ocr_confidence is <= this (default 0.55, RecognizeDocumentsRequest scale); above, Vision is good enough (vision_ok_skip)."),
  ...
  async (args) => {
    ...
    if (args.no_dedup_content) a.push("--no-dedup-content");
...
server.registerTool(
  ...
  {
    description: "Scan the daily Apple Notes export (~/Archives/Notes/, produced by mac-automations via anotes) and list markdown files to copy into the knowledge base. Already-ingested notes are resolved through tracking.db (current transcription path, body hash) — NOT through the mirror path, which `organize apply` moves; a known note is `modifie` only if its body changed (status breakdown in `by_status`: nouveau / modifie / manquante). Returns an `export` freshness probe ({last_export, age_days, stale}) — if `stale` is true, the export job is not running and the listing reflects an outdated snapshot. " +
