...
| `doc_signals` | `rel_path` (relatif à `~/Documents`) | **fiche d'identité, étage signaux** (Phase B) : paquet JSON nom/chemin/dates/métadonnées/texte born-digital + **`excerpt`** (extrait brut, signal premier du classement) + **`pages`** (nb de pages PDF, filtre de coût de la repasse Mistral) + résumé extractif, caché par `(rel_path, size, mtime)` et invalidé par **version de schéma** `_v` (v5). |
